function [params] = read_scan_params(tmpDir) %% Read ScanParams.txt and ProcOpts.txt
    rootDir = '.';
    if nargin<1
        tmpDir = rootDir;
    end
    ...
    params.x = scan_len *1e3; % mu
    
    [idx, idx2] = regexp(scanprms_txt, 'lengthOffset\=');
    len_txt = scanprms_txt(idx2+1:idx2+20);
    len_txt = strtok(len_txt);
    x0 = str2double(len_txt);
    params.x0 = x0 *1e3; % mu
    ...
    params.y = scan_width*1e3;%mu
    
    [idx, idx2] = regexp(scanprms_txt, 'widthOffset\='); % width will be 0 (widthSteps = 1) for 2-d cross-sections
    width_txt = scanprms_txt(idx2+1:idx2+20);
    width_txt = strtok(width_txt);
    scan_widthOffset = str2double(width_txt);
    params.y_offset = scan_widthOffset*1e3;%mu
    ...
    [idx, idx2] = regexp(scanprms_txt, 'fastScanFreq\=');
    fastScanFreq = scanprms_txt(idx2+1:idx2+20);
    fastScanFreq = strtok(fastScanFreq);
    fastScanFreq = str2double(fastScanFreq);
     params.fastScanFreq = fastScanFreq;
